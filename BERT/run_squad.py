...
def main():
    ...
    parser.add_argument('--gpu-rank', default=0, type=int,
    ...
    if args.world_size == 1 or args.no_cuda:
        ...
        device = torch.device("cuda", args.gpu_rank)
        n_gpu = 1
        # Initializes the distributed backend which will take care of sychronizing nodes/GPUs
        torch.distributed.init_process_group(backend='nccl', world_size=args.world_size,
                                             init_method=args.dist_url, rank=args.rank)
    ...
    model = BertForQuestionAnswering.from_pretrained(args.bert_model,
                cache_dir=os.path.join(str(PYTORCH_PRETRAINED_BERT_CACHE), 'distributed_{}'.format(args.rank)))
    ...
    if args.world_size != 1:
        try:
            from torch.apex.parallel import DistributedDataParallel as DDP
        except ImportError:
    ...
    if args.do_predict and (args.world_size == 1 or torch.distributed.get_rank() == 0):
